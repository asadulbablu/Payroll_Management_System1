...
@Repository
public class BasicYearServiceImpl implements BasicYearService{
    ...
    public Integer deleteBasicYear(int id) {
       Session s = sessionFactory.openSession();
        Transaction t = s.getTransaction();
        t.begin();
        BasicYear cm = (BasicYear) s.get(BasicYear.class, id);
        s.delete(cm);
        t.commit();
        s.close();
    ...
    public String viewBasicYear() {
      Session s = sessionFactory.openSession();
        Transaction t = s.getTransaction();
        t.begin();
        List<BasicYear> categorieslist = s.createQuery("from BasicYear").list();
        Gson g = new Gson();
