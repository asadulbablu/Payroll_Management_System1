...
<html>
    <head>
        ...
        <script>
            var loanpaylist = ${loanpaymentlist};
            var emplist = ${empIlist};
            var emploanlist = ${emploantlist};
            angular.module('organocartpackage', []).controller('ProductController',
                    function ($scope) {
                    $scope.lpayl = loanpaylist;
                    $scope.empl = emplist;
                    $scope.eloan = emploanlist;
    ...
    <body ng-app="organocartpackage" ng-controller="ProductController">	
                    ...
                    <div class="inner-block">
                        ...
                        <div class="market-updates">
                            <div class="formcontainer">
                                <div class="pagetitle" align="center"><h4>Add Loan Pay</h4></div>
                                <form:form commandName="loanpayObject" action="addingloanpay"
                                           enctype="multipart/form-data">
                                    <c:if test="${check}">
                                        <div class="form-group">

                                            <form:input class="form-control" Placeholder="Loan Pay Id"
                                                        type="text" path="loanpayid" required="true"></form:input>
                                            </div>
                                    </c:if>
                                    <c:if test="${!check}">
                                        <div class="form-group">
                                            <form:input class="form-control" Placeholder="Loan Pay Id"
                                                        type="text" path="loanpayid" readonly="true" required="true"></form:input>
                                            </div>
                                    </c:if>
                                    <div class="form-group">
                                        <select class="form-control" ng-model="selectedSup" ng-options="x.empname for x in empl">
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <select class="form-control" ng-model="selectedSup1" ng-options="x.loanid for x in eloan">
                                            </select>
                                        </div>
                                        <div class="form-group">
                                        <form:hidden  class="form-control" value="{{selectedSup1.loanid}}" path="loanid"></form:hidden>
                                        </div>
                                        <div class="form-group">
                                        <form:input  class="form-control" value="{{selectedSup1.loanamount}}" path="loanamount"></form:input>
                                        </div>
                                        <div class="form-group">
                                        <form:input class="form-control" Placeholder="Loan Pay Amount"
                                                    type="number" path="loanpayamount" required="true"></form:input>
                                        </div>
                                        <div class="form-group">
                                        <form:input type="text" class="form-control"
                                                    placeholder="Loan pay Date" path="loanpaydate" required="true"></form:input>
                                        </div>
                        ...
                        <div class="chit-chat-layer1">
                            <div class="card-body" >
                                <span class="lead">List Of Loan Pay</span>
                                <div width="100%">
                                    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                                        <thead>
                                            <tr>
                                                <th>Loan Pay Id</th>
                                                <th>Loan Pay Amount</th>
                                                <th>Loan Pay Date</th>
                                                <th>Employee Id</th>
                                                <th>Loan Id</th>
                                                <th>Loan Amount After Installment</th>
                                            </tr>
                                        </thead>

                                        <tbody>
                                            <tr ng-repeat="p in lpayl">

                                                <td>{{p.loanpayid}}</td>
                                                <td>{{p.loanpayamount}}</td>
                                                <td>{{p.loanpaydate}}</td>
                                                <td>{{p.empid}}</td>
                                                <td>{{p.loanid}}</td>
                                                <td>{{p.loanamountain}}</td>
                                                <td><a href="editloanpay?getpid={{p.loanpayid}}"><button id="editbuttons" type="submit" class="btn btn-warning">Edit</button></a> 
                                                    <a href="removingloanpay/{{p.loanpayid}}"><button id="removebuttons" type="button" class="btn btn-danger">Remove</button></a>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
        ...
        <script>
            $(document).ready(function () {
            var date_input = $('input[name="loanpaydate"]'); //our date input has the name "date"
